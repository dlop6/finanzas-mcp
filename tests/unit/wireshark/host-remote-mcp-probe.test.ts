...
describe("Host remote MCP Wireshark probe", () => {
  it("uses the Host client lifecycle, discovers 30 tools, calls one read, and creates a safe summary", async () => {
    ...
    expect(summary).toMatchObject({ toolCount: 30, readTool: "get_current_balance", transport: "STREAMABLE_HTTP", lifecycleValidated: true });
