...
export function createFinancialReportDemo(options: CreateFinancialReportDemoOptions): { run(): Promise<FinancialReportDemoResult> } {
  ...
  return {
    async run() {
      ...
      if (allTools.length !== 53 || allTools.filter((tool) => tool.isWriteOperation).length !== 26) throw new FinancialReportDemoError("INVALID_CATALOG", "The local MCP catalog is incomplete.");
