...
export function createFinancialReportDemo(options: CreateFinancialReportDemoOptions): { run(): Promise<FinancialReportDemoResult> } {
  ...
  return {
    async run() {
      ...
      if (allTools.length !== 56 || allTools.filter((tool) => tool.isWriteOperation).length !== 27) throw new FinancialReportDemoError("INVALID_CATALOG", "The local MCP catalog is incomplete.");
