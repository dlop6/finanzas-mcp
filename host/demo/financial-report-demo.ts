...
export function createFinancialReportDemo(options: CreateFinancialReportDemoOptions): { run(): Promise<FinancialReportDemoResult> } {
  ...
  return {
    async run() {
      ...
      if (allTools.length !== 50 || allTools.filter((tool) => tool.isWriteOperation).length !== 24) throw new FinancialReportDemoError("INVALID_CATALOG", "The local MCP catalog is incomplete.");
