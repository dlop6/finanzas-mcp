...
export async function runRemoteMcpProbe(endpointValue: string, request: RemoteMcpProbeFetch = globalThis.fetch): Promise<void> {
  ...
  try {
    ...
    if (!Array.isArray(definitions) || definitions.length !== 27 || "error" in tools) throw new RemoteMcpProbeError("INVALID_RESPONSE");
...
if (isEntrypoint) {
  runRemoteMcpProbe(process.argv[2] ?? "")
    .then(() => process.stdout.write("Remote MCP capture probe completed: 27 tools and one read operation.\n"))
    .catch((error: unknown) => {
