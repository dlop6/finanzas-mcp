...
import { CashFlowService, CurrentBalanceService, DebtService, InventoryService, ProjectionService, PurchaseViabilityService, ReceivableService, SaleService, TransactionReferenceService, TransactionService, type FinanceClock } from "./services";
...
import { createSaleTools } from "./tools/sale-tools";
...
export function createFinanceToolRegistry(prisma: PrismaClient, options: FinanceToolRegistryOptions = {}): FinanceToolRegistry {
  ...
  const sales = new SaleService(repositories.sales);
  ...
  return new FinanceToolRegistry([...createTransactionTools(transactions), ...createDebtTools(debts), ...createReceivableTools(receivables), ...createInventoryTools(inventory), ...createSaleTools(sales), ...createCashFlowTools(balance, cashFlow), ...createProjectionTools(projection), ...createPurchaseViabilityTools(purchaseViability), ...createTransactionReferenceTools(transactionReferences)]);
}
