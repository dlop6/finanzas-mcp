...
import { CashFlowService, CurrentBalanceService, DebtService, InventoryService, ProjectionService, ReceivableService, TransactionService } from "./services";
...
export function createFinanceToolRegistry(prisma: PrismaClient): FinanceToolRegistry {
  ...
  return new FinanceToolRegistry([...createTransactionTools(transactions), ...createDebtTools(debts), ...createReceivableTools(receivables), ...createInventoryTools(inventory), ...createCashFlowTools(balance, cashFlow), ...createProjectionTools(projection)]);
}
